...
def parse_ayvens_document(
    ...
    if doc_type == "rent":
        ...
        for item in parsed_lines_extra:
            ...
            if tpl:
                amount = item["net"] if item["net"] else item["total"]
                row = build_ayvens_row_from_template(tpl, period, amount, "EXTRA")

                is_flex_extra = "ayvens flex" in normalize_text(item.get("description", "")) or "flexiplan" in normalize_text(item.get("description", ""))
                if is_flex_extra:
                    row["description"] = item["plate"]
                    row["produit"] = "LOCATION"
                    row["prodfourn"] = "LOCATION"
                    row["unite"] = "US"
                    row["compte"] = "62612100"
                    row["ana1"] = "9"
                    row["project"] = "DO5015"
                    ...
                    row["ct"] = "NV"
                    row["code_iva"] = "NV"
                    row["iva"] = str(item.get("iva_pct", 23))
                else:
