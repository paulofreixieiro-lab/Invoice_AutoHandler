...
APP_VERSION = "Prod 1.0"

EDP_SUMMARY_COLUMNS = [
    "InvoiceNumber", "CA", "Estado", "kWh", "AV (€)", "Valor(€)",
    "Periodo", "DocType", "PdfFile", "FinalFile", "ProcessedAt"
]
...
def register_processed_invoice(supplier: str, invoice_key: str, file_hash: str, invoice_number: str,
    ...
    cur.execute("""
        INSERT INTO processed_invoices (
            ...
            ca_used, source_filename, final_filename, processed_at
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        supplier, invoice_key, file_hash, invoice_number, period, doc_type, ca_used,
        source_filename, final_filename, datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ))
    conn.commit()
    conn.close()
...
def ensure_excel():
    ...
    with pd.ExcelWriter(EXCEL_FILE, engine="openpyxl") as writer:
        ...
        pd.DataFrame(columns=[
            "Supplier", "CA", "Periodo", "InvoiceNumber", "DocType", "RowsCount",
            "TotalValorHT", "Estado", "PdfFile", "FinalPdf", "ProcessedAt"
        ...
        pd.DataFrame(columns=[
            "Supplier", "CA", "Periodo", "InvoiceNumber", "DocType", "RowsCount",
            "TotalValorHT", "Estado", "PdfFile", "FinalPdf", "FinalXml", "ProcessedAt"
...
def build_ayvens_row_from_template(template: dict[str, str], period: str, amount: float, row_type: str) -> dict[str, Any]:
    ...
    ct = template.get("ct", template.get("CT", "")).strip().upper()
...
def parse_ayvens_document(pdf_path: Path, ayvens_admin: dict[str, str]) -> InvoiceRecord:
    ...
    if doc_type == "rent":
        ...
        for item in parsed_lines:
            ...
            rent_amount = item["locacao"] * 1.23 if plate_norm in AYVENS_SPECIAL_FULL_VAT_RENT_PLATES else item["locacao"]
            service_vat_amount = item["exploracao_sujeita"] * 1.23 if item["exploracao_sujeita"] else 0.0
            service_exempt_amount = item["isento"]
            if rent_amount > 0:
                tpl = template_index.get((plate_norm, "RENT")) or build_ayvens_fallback_template(plate, "RENT", generic_index, galp_defaults)
                if tpl:
                    rows.append(build_ayvens_row_from_template(tpl, period, rent_amount, "RENT"))
                else:
                    missing.append(f"Linha RENT sem template para {plate}")
            if service_vat_amount > 0:
                tpl = template_index.get((plate_norm, "SERVICE_VAT")) or build_ayvens_fallback_template(plate, "SERVICE_VAT", generic_index, galp_defaults)
                if tpl:
                    rows.append(build_ayvens_row_from_template(tpl, period, service_vat_amount, "SERVICE_VAT"))
                else:
                    missing.append(f"Linha SERVICE_VAT sem template para {plate}")
            if service_exempt_amount > 0:
                tpl = template_index.get((plate_norm, "SERVICE_EXEMPT")) or build_ayvens_fallback_template(plate, "SERVICE_EXEMPT", generic_index, galp_defaults)
                if tpl:
                    rows.append(build_ayvens_row_from_template(tpl, period, service_exempt_amount, "SERVICE_EXEMPT"))
                else:
        ...
        for item in parsed_lines:
            plate = item["plate"]
            amount = item["total"]
            tpl = template_index.get((normalize_plate(plate), "EXTRA")) or generic_index.get("EXTRA") or build_ayvens_fallback_template(plate, "EXTRA", generic_index, galp_defaults)
            if tpl:
                ...
                rows.append({
                    ...
                    "prodfourn": ayvens_admin.get("extra_prodfourn", ayvens_admin.get("extra_produit", "PORTAGENS")),
...
class FaturasFacilitiesV12(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.user_info_var = tk.StringVar(value=f"Utilizador local: {self.current_user} | Perfil: {self.user_role}")
    ...
    def _build_history_tab(self):
        cols = ("supplier", "invoice", "period", "ca", "final", "processed")
        ...
        headers = {
            ...
            "ca": "CA", "final": "Ficheiro Final", "processed": "Processado em"
        }
        for c in cols:
            self.tree_history.heading(c, text=headers[c])
            self.tree_history.column(c, width=220 if c == "final" else 140, anchor="w")
        self._pack_tree(body, self.tree_history)

    def _build_maps_tab(self):
        frame = ttk.Frame(self.tab_maps, padding=10)
        frame.pack(fill="both", expand=True)
        ttk.Label(frame, text="Ficheiros de gestão activos na pasta .bin").pack(anchor="w")
    ...
    def _build_errors_tab(self):
        ...
        for c, h, w in [("supplier", "Supplier", 120), ("file", "Ficheiro", 260), ("error", "Erro", 1000)]:
            ...
            self.tree_errors.column(c, width=w, anchor="w")
    ...
    def _build_galp_tab(self):
        ...
        ttk.Label(tab, textvariable=self.galp_status).pack(anchor="w", padx=10)
    ...
    def _build_delta_tab(self):
        ...
        ttk.Label(tab, textvariable=self.delta_status).pack(anchor="w", padx=10)
        ...
        for c in cols:
            tree.heading(c, text=hdr.get(c, c))
            tree.column(c, width=150 if c == "description" else 120, anchor="w")
        self._pack_tree(body, tree)
    ...
    def _build_samsic_tab(self):
        ...
        ttk.Label(tab, textvariable=self.samsic_status).pack(anchor="w", padx=10)
        ...
        for c in cols:
            tree.heading(c, text=hdr.get(c, c))
            tree.column(c, width=130 if c == "description" else 100, anchor="w")
        self._pack_tree(body, tree)
    ...
    def _build_evio_tab(self):
        ...
        ttk.Label(tab, textvariable=self.evio_status).pack(anchor="w", padx=10)
        ...
        for c in cols:
            tree.heading(c, text=hdr.get(c, c))
            tree.column(c, width=160 if c == "description" else 110, anchor="w")
        self._pack_tree(body, tree)
    ...
    def _build_viaverde_tab(self):
        ...
        ttk.Label(tab, textvariable=self.viaverde_status).pack(anchor="w", padx=10)
    ...
    def _build_ayvens_tab(self):
        ...
        ttk.Label(tab, textvariable=self.ayvens_status).pack(anchor="w", padx=10)
        ...
        for c in cols:
            tree.heading(c, text=hdr.get(c, c))
            tree.column(c, width=150 if c in ("description","produit","prodfourn") else 110, anchor="w")
        self._pack_tree(body, tree)
    ...
    def populate_history_tree(self):
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT supplier, invoice_number, period, ca_used, final_filename, processed_at
    ...
    def process_simple_supplier(self, supplier: str, records: list[InvoiceRecord]):
        ...
        for rec in to_process:
            ...
            register_processed_invoice(supplier, rec.invoice_key, rec.file_hash, rec.invoice_number, rec.period,
                                       rec.doc_type, rec.ca, rec.file_name, rec.final_name)
    ...
    def process_galp(self):
        ...
        register_processed_invoice("GALP", doc.invoice_key, doc.file_hash, doc.invoice_number, period,
                                   doc.doc_type, ca, doc.file_name, pdf_new.name)
    ...
    def process_delta(self):
        ...
        summary_df = pd.DataFrame([{
            ...
            "Estado": "Processado", "PdfFile": doc.file_name,
            "FinalPdf": final_name, "ProcessedAt": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        }])
        output_df = pd.DataFrame([{
            "Description": r.get("description", ""),
            "Produit": r.get("produit", ""),
    ...
    def process_evio(self):
        ...
        register_processed_invoice("EVIO", doc.invoice_key, doc.file_hash, doc.invoice_number, period,
                                   doc.doc_type, ca, doc.file_name, final_pdf_name)
    ...
    def process_viaverde(self):
        ...
        register_processed_invoice("VIAVERDE", doc.invoice_key, doc.file_hash, doc.invoice_number, period, doc.doc_type, ca, doc.file_name, ", ".join(fp.name for fp in final_pdfs))
        ...
        summary_df = pd.DataFrame([{
            ...
            "FinalXml": doc.source_path.name, "ProcessedAt": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }])
    ...
    def process_ayvens(self):
        ...
        register_processed_invoice("AYVENS", doc.invoice_key, doc.file_hash, doc.invoice_number, period,
                                   doc.doc_type, ca, doc.file_name, final_name)
    ...
    if doc_type == "rent":
        ...
        for item in parsed_lines_rent:
            ...
            amt_map = {
                ...
                "SERVICE_VAT": item["exploracao_sujeita"] * 1.23 if item["exploracao_sujeita"] else 0.0,
        ...
        for item in parsed_lines:
            ...
            if tpl:
                ...
                row["description"] = item["description"]
                row["iva"] = str(item.get("iva_pct", 0))
                row["code_iva"] = tpl.get("ct", row.get("code_iva", "BR"))
                rows.append(row)
            else:
